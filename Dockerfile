...
RUN echo export CUDA_ROOT=/usr/local/cuda-9.0 >>/etc/profile && \
	...
	echo export CUDNN_PATH=/usr/lib/x86_64-linux-gnu/libcudnn.so.7 >>/etc/profile && \
	echo export CUDNN_PATH=/usr/lib/x86_64-linux-gnu/libcudnn.so.7 >>/etc/bash.bashrc && \
